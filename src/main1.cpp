...
int main()
{
	...
	cout << "rotate matrix: "<< rotM <<endl;
	cout << "transform matrix"<< tvec << endl;
	...
	ofstream fout("../data/pnp_theta.txt");
	fout << -1 * thetax << endl << -1 * thetay << endl << -1 * thetaz << endl;
	cout << "camera:" << -1 * thetax << ", " << -1 * thetay << ", " << -1 * thetaz << endl;
	fout.close();
	/*************************************此处计算出相机的旋转角END**********************************************/
	...
	//利用求逆的方式
	cv::Mat test = -rotM.inv()*tvec;
	cout << "word1:"<<test<<endl;
